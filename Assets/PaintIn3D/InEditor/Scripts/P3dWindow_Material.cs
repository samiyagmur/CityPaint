...
namespace PaintIn3D
{
	public partial class P3dWindow
	{
		...
		private P3dMaterialColoner materialInstance;
		...
		private void LoadMaterial(P3dMaterialColoner prefab)
		{
			ClearMaterial();
		...
		private void DrawMaterial()
		{
			...
			foreach (var cachedMaterial in P3dMaterialColoner.CachedMaterials)
			{
			...
			materialScrollPosition = GUILayout.BeginScrollView(materialScrollPosition, GUILayout.ExpandHeight(true));
				...
				if (selected != null)
				{
					LoadMaterial((P3dMaterialColoner)selected); selectingMaterial = false;
				}
			...
			EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
				...
				{
					P3dMaterialColoner.ClearCache(); AssetDatabase.Refresh();
				}
